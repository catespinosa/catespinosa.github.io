...
$(document).ready(function () {
    ...
    $('#clickme').click(function(event){
      ...
      if(input.val() == "") {
        ...
        addCompleteLinks(item); //add span to existing list before adding the
        list.append(item);
...
function addCompleteLinks(li) {
  var $archiveLink = $('<span>').html(' archive - ').addClass('complete-task');//create the span\text
  $archiveLink.on('click',function(event) {
   $(this).parent().remove();
  });
  li.prepend($archiveLink);//prepend span to li
}
